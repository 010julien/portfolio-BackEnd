...
export class DashboardService {
  ...
  async getStats() {
    ...
    const completedProjects = await this.projectsService.countByStatus(ProjectStatus.COMPLETED);
    const inProgressProjects = await this.projectsService.countByStatus(ProjectStatus.IN_PROGRESS);
    const futureProjects = await this.projectsService.countByStatus(ProjectStatus.FUTURE);
    ...
    const avgSkillPercentage = skills.length > 0
    ...
    const skillCategoryCounts = Object.entries(skillsByCategory).map(([category, skills]) => ({
      category,
      count: skills.length,
    }));
